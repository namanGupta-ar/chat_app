...
export default function Post(){
    ...
    const submitPost = async (e) => {
        ...
        if(!post.description){
            // todo - avoid continuous clicking
        ...
        <div className="my-20 p-12 shadow-lg rounded-lg max-w-md mx-auto">
            <form onSubmit={submitPost}>
                ...
                <div className="flex justify-between">
                    <button type="submit" 
                    className="w-full m-2 bg-cyan-600 text-white font-medium p-4 rounded-lg text-sm">Submit</button>
                    {post.hasOwnProperty("id") ? 
                        <button type="reset" onClick={cancelPost}
                        className="w-full m-2 bg-red-800 text-white font-medium p-4 rounded-lg text-sm">Cancel</button> : ""
                    }
